...
function init2DArray(){
    ...
    let maxcolumn = grid.lenght;
     for(_columns = 0; _columns< maxcolumn; _columns++){
        maxrows = grid[_columns].lenght;
...
function unsetHighlights(){
  for(let i1=0; i1<grid.lenght; i1++){
    for(let i2=0; i2<grid[i1].lenght; i2++){
      grid[i1][i2].tiHighlight = false;
    }
  }
}
...
function createDomElements(){
   restertButton = createButton("RESTART");
...
function createPieces(){
  for(let i1=0; i1<grid.length; i1++){
    for(let i2=0; i2<grid[i1].length; i2++){
      ...
      let pawnW = new PawnW(grid[i1][i2].i , grid[i1][i2].j , imgB[0] );
       pieces.push(pawnW);
    }
    else if(grid[i1][i2].rookW){
      let rookW = new RookW(grid[i1][i2].i , grid[i1][i2].j , imgB[1] );
       pieces.push(rookW);
    }
    else if(grid[i1][i2].knightW){
      let knightW = new KnightW(grid[i1][i2].i , grid[i1][i2].j , imgB[2] );
       pieces.push(knightW);
    }
    else if(grid[i1][i2].bishopW){
      let bishopW = new BishopW(grid[i1][i2].i , grid[i1][i2].j , imgB[3] );
       pieces.push(bishopW);
    }
    else if(grid[i1][i2].queenW){
      let queenW = new QueenW(grid[i1][i2].i , grid[i1][i2].j , imgB[4] );
       pieces.push(queenW);
    }
    else if(grid[i1][i2].kingW){
      let kingW = new KingW(grid[i1][i2].i , grid[i1][i2].j , imgB[5] );
       pieces.push(kingW);
    }
  }
 }   
